...
def load_checkpoint(model, optimizer, filename, map_location, logger=None):
    if os.path.isfile(filename):
        ...
        if model is not None and checkpoint['model_state'] is not None:
            ...
            # model.load_state_dict(checkpoint['model_state']) # OG 코드
        if optimizer is not None and checkpoint['optimizer_state'] is not None:
            # optimizer.load_state_dict(checkpoint['optimizer_state']) # OG 코드
            # 추가된 코드 srt
            try:
                optimizer.load_state_dict(checkpoint['optimizer_state'])
            except ValueError as e:
                if logger:
                    logger.warning(f"Optimizer state_dict loading skipped: {e}")
            # 추가된 코드 end
