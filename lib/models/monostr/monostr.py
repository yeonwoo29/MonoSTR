...
from .KP_pos import build_keypoint_position_tensor
def _get_clones(module, N):
    return nn.ModuleList([copy.deepcopy(module) for i in range(N)])


class MonoSTR(nn.Module):
    """ This is the MonoSTR module that performs monocualr 3D object detection """
    def __init__(self, backbone, depth_predictor, det2d_transformer, det3d_transformer,
        ...
        # 키포인트 위치 임베딩을 위한 프로젝션 레이어 (256 -> 256)
        self.kp_pos_proj = nn.Linear(256, hidden_dim)
    ...
    def forward(self, images, calibs, targets, img_sizes, dn_args=None):
        ...
        if isinstance(targets, list) and 'keypoints' in targets[0]:
            kp_list = [t['keypoints'] for t in targets]
            kp_tensor = torch.stack(kp_list, dim=0).to(query_embeds.device)  # (bs, num_queries, 24)
            kp_tensor = kp_tensor.repeat(1, self.group_num, 1) # (bs, num_queries*group_num, 24)
            kp_pos_embed = build_keypoint_position_tensor(kp_tensor)  # (bs, 550, 256)
            
            # 키포인트 위치 임베딩을 3D transformer에 전달하기 위해 차원 변환
            kp_pos_embed = self.kp_pos_proj(kp_pos_embed)  # (bs, 550, 256)
            
            # 쿼리 임베딩은 그대로 사용 (키포인트 정보는 cross attention에서 처리)
            query_kp_embed = query_embeds
        ...
        hs, init_reference, inter_references = self.det3d_transformer(intermediate_output, query_kp_embed, depth_pos_embed, keypoint_pos_embed=kp_pos_embed)
...
class SetCriterion(nn.Module):
    """ This class computes the loss for MonoSTR.
