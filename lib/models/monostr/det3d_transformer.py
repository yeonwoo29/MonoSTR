...
class Det3DTransformer(nn.Module):
    ...
    def forward(self, intermediate_output, query_embeds, depth_pos_embed, keypoint_pos_embed=None):
        ...
        # 키포인트 위치 임베딩 차원 변환 (B, Q, D) -> (Q, B, D)
        if keypoint_pos_embed is not None:
            keypoint_pos_embed = keypoint_pos_embed.transpose(0, 1)  # (Q, B, D)
        ...
        hs, inter_references = self.decoder(
            ...
            mask_depth, 
            bs=bs,
            keypoint_pos_embed=keypoint_pos_embed)
...
class DepthAwareDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        # keypoint cross attention (새로 추가)
        self.cross_attn_keypoint = nn.MultiheadAttention(d_model, n_heads, dropout=dropout)
        self.dropout_keypoint = nn.Dropout(dropout)
        self.norm_keypoint = nn.LayerNorm(d_model)
    ...
    def forward(self,
                ...
                depth_pos_embed,
                mask_depth,
                bs=None,  # bs를 기본값으로 변경
                keypoint_pos_embed=None):  # keypoint_pos_embed를 마지막으로
        ...
        # keypoint cross attention (새로 추가)
        if keypoint_pos_embed is not None:
            tgt2 = self.cross_attn_keypoint(tgt.transpose(0, 1),
                                           keypoint_pos_embed,
                                           keypoint_pos_embed)[0].transpose(0, 1)
            tgt = tgt + self.dropout_keypoint(tgt2)
            tgt = self.norm_keypoint(tgt)
...
class DepthAwareDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
                ...
                keypoint_pos_embed=None):  # keypoint_pos_embed를 마지막으로 이동
        output = tgt
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output = layer(output,
                           ...
                           mask_depth,
                           bs,
                           keypoint_pos_embed)  # keypoint_pos_embed 전달
