...
class KITTI_Dataset(data.Dataset):
    def __init__(self, split, cfg):
        ...
        ########################################################################## pedestrian -> car
        self.writelist = cfg.get('writelist', ['Pedestrian'])
        ...
        # ◀ KEYPOINTS:
        self.keypoints_path = cfg.get('keypoints_path', None)
        if self.keypoints_path:
            with open(self.keypoints_path, 'r') as f:
                kp_list = json.load(f)
            self.kp_map = {}
            for entry in kp_list:
                img_file = entry['image_id']
                idx_int = int(os.path.splitext(img_file)[0])
                obj_idx = int(entry['object_index'])
                kps = np.array(entry['keypoints'], dtype=np.float32).flatten()  # (24,)
                self.kp_map.setdefault(idx_int, {})[obj_idx] = kps
        else:
            self.kp_map = {}
...
######################### Pedestrian
    def eval(self, results_dir, logger):
        ...
        for category in self.writelist:
            ...
            if category == 'Pedestrain': ################################pedestrian -> car
                ped_moderate = mAP3d_R40
    ...
    def __getitem__(self, item):
        ...
        # ◀ KEYPOINTS: build padded keypoints array
        keypoints = np.zeros((self.max_objs, 24), dtype=np.float32)
        if index in self.kp_map:
            for obj_idx, kps in self.kp_map[index].items():
                if obj_idx < self.max_objs:
                    keypoints[obj_idx] = kps
        # image loading
